...
export default function PortalPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    async function fetchRegistration() {
      try {
        const { data: regData, error } = await supabase.rpc('get_registration_by_token', { token });

        if (error) throw error;
        if (!regData || regData.length === 0) {
          throw new Error("Registration not found or link is invalid.");
        }

        setData(regData[0]);
        setFormData(regData[0].form_responses || {});
      } catch (err: any) {
        console.error("Fetch error:", err);
        setErrorMsg(err.message || "Failed to load registration.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update local data state so View Mode shows new answers
            ...
            <form onSubmit={handleSave} className="space-y-6">
                ...
                <div key={field.id} className="space-y-2">
                  ...
                  {field.type === 'textarea' && (
                    <textarea
                      ...
                      value={formData[field.id] || ""}
                      rows={4}
                      className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-zinc-900 dark:text-zinc-100 resize-none"
                      onChange={e => handleChange(field.id, e.target.value)}
                    />
                  )}
                  ...
                  {field.type === 'select' && (
                    <select
                      required={field.required}
                      value={formData[field.id] || ""}
                      className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-zinc-900 dark:text-zinc-100"
                      onChange={e => handleChange(field.id, e.target.value)}
                    >
              ...
              <div className="flex gap-3 pt-6 border-t border-zinc-100 dark:border-zinc-800">
                <button
                  ...
                  onClick={() => {
                    setIsEditing(false);
                    setFormData(data.form_responses || {}); // Revert changes
                  }}
